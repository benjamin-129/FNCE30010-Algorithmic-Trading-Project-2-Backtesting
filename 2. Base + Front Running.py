...
def initialize(context):
    ...
    schedule_function(my_assign_weights_p98, date_rules.month_end(days_offset=5), time_rules.market_open())
    schedule_function(my_rebalance, date_rules.month_end(days_offset=5), time_rules.market_open(hours=1))
...
# XLV
context.health = health = sid(19661)
# ITA
context.defence = defence = sid(32013)

 

context.stock_leverage = 1
sec_list = [mom1, mom2, tbill, agg, tech, health, defence]
...
def my_assign_weights(context, data):
    context.weights = pd.Series(index=context.output.index)
    returns = context.output['yr_returns']
    sharpes = context.output['sharpe']

    # comparing tech, health and defence: choose highest sharpe
    if sharpes[context.tech] > sharpes[context.health]:
        if sharpes[context.tech] > sharpes[context.defence]:
            context.industry = context.tech
        else:
            context.industry = context.defence
    else:
        if sharpes[context.health] > sharpes[context.defence]:
            context.industry = context.health
        else:
            context.industry = context.defence
    # Actual weighting
    if returns[context.industry] > returns[context.mom1]:
        if returns[context.industry] > returns[context.tbill]:
            context.weights[context.industry] = context.stock_leverage
        else:
            context.weights[context.agg] = 1
